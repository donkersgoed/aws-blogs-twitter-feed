...
def get_twitter_api():
    """Retrieve the Twitter Consumer key and secret from AWS Secrets Manager."""
...
def prepare_twitter_text(ddb_item):
    ...
    removed_chars = 0
    shortened_suffix = ' […]'
    while (title_len - removed_chars) > rest_len_for_title + len(shortened_suffix):
...
def prepare_authors(authors: List[str]):
    """Take a list of authors, convert them to twitter handles, and add commas."""
    with open('twitter_handles.yaml') as twitter_handles_file:
        twitter_handles = yaml.load(twitter_handles_file, Loader=yaml.FullLoader)
